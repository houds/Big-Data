...
shinyServer(function(input, output) {
    ...
    output$dataTable = renderDataTable({
        data=brute =read.csv("monfichier.csv",header = TRUE, sep= ";", encoding="UTF-8") 
    ...
    output$tree1 <- renderPlot({
        cible=SeriousDlqin2yrs
        fit26 <-rpart(cible ~ DELINQ+DEROG, bdd)
        rpart.plot(fit26, roundint=FALSE)
    })
    ...
    r1 <- eventReactive(input$button2,{ 
        cible=SeriousDlqin2yrs
        newtrain=train
        set.seed(1234)
        X <- data.matrix(subset( newtrain , select =-c(cible)))
