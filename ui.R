             ...
             dans ces deux echantillons est similaire :"),
              ...
              efficace pour la prédiction."),
                     ...
                     tabPanel("Conclusion", br(),
                              ...
                              p("Malgré le développement et la diffusion de nombreux algorithmes efficaces
                        de classification par apprentissage machine, le modèle de notation de référence
                          dans l'industrie du crédit reste la régression logistique. Cet état de fait est principalement dû à la stabilité et à la robustesse du
                        modèle de régression logistique, ainsi qu'à sa capacité d'interprétation intrinsèque."),
                              p("De nombreux articles universitaires préconisent l'utilisation de méthodes d'ensemble
                      plus sophistiquées, telles que la méthode de Random Forest. Mais, ces modèles de la boîte 
                      noire ne sont pas interprétables."),
                              p("Reconnaissant que la régression logistique traditionnelle soit moins performante
                      que le Random Forest en raison de ses pièges dans la modélisation des effets non 
                      linéaires."),
                              
                              p("On a présenté dans cette application, la régression par arbre logistique pénalisé (PLTR) 
                      avec des variables prédictives données par des effets de seuil endogènes univariés et bivariés
                      faciles à interpréter. Ces effets sont quantifiés par des variables fictives associées aux nœuds
                      des feuilles des arbres de décision de courte profondeur construits avec des couples de 
                      variables prédictives originales. "),
                              
                              p("Il convient également pour la méthode PLTR, tout comme par exemple le SVM ou le Random 
                      Forest, de fixer les hyper-paramètres de façon optimale. Un mauvais choix dans ces 
                      hyper-paramètres peut conduire à un modèle très peu performant.") )
